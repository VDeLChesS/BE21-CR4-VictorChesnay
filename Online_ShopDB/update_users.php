<?php
    ...
    if (isset($_POST["update"])) {
        ...
        if ($_FILES["picture"]["error"] == 4) {
            $sql = "UPDATE `users` SET `first_name`='{$first_name}',`last_name`='{$last_name}',`date_of_birth`='{$date_of_birth}',`email`='{$email}', `address`= '{$address}' WHERE `id` = '{$id}'";
        } else {
            if ($row["picture"] != "avatar.png") {
                unlink("pictures/{$row["picture"]}");
            }
            $sql = "UPDATE `users` SET `first_name`='{$first_name}',`last_name`='{$last_name}',`date_of_birth`='{$date_of_birth}',`email`='{$email}', `picture`='{$picture[0]}', `address`= '{$address}' WHERE `id` = '{$id}'";
        }
...
<html lang="en">
    ...
    <body>
        ...
        <header>
            <h1 class="text-center">Edit User Profil Information</h1>
        </header>
